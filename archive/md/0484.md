...
- Date: Apr 16 8:27 AM
- Original: [https://groups.yahoo.com/neo/groups/blupf90/conversations/messages/484](https://groups.yahoo.com/neo/groups/blupf90/conversations/messages/484)

```
 I bet for a numerical problem. I strongly recommend that instead of using the dominance matrix in "outfile", you
invert it with an external problem (e.g. "solve' in R) and check the eigenvalues. All should be positive,Â  and if not
do something like 0.99D+0.01 I . Then use the file with the inverted matrix and option user_file (not user_file_inv).
...
animals. You must code the dominant effects from 1 to 3455 and then put those values as an extra column in the data
