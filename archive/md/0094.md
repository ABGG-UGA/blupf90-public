...
Le 06/12/2012 20:01, Mohamed a écrit :
 ...
 I need to preform a covariance in the renumf90 parameter file, I dont understand how to use the NESTED order in Ignacy's notes.

 It depends whether you want to "nest" your covariate (e.g., the slope depends on the sex of the animal)or not. If it'sn not nested, don't put anything.
 If it's nested write
 NESTED
 d1 ... dn
 where nested is the column indicating the levels for "nesting", e.g., it would be the column with the sex (in my example)


 I added year of birth in my data file, and it is also on my pedigree file, should I make the year of birth in the 4 digit format, or should I change it into another sequence?
 ...
 >
 > Mohamed,
 > It looks to me that you have a problem with the data file rather than with the pedigree one. Renum cannot find animals with records while it sees the pedigree. Follow the remarks of Andres but in the data.
 > 
 > OPTION alpha_size?
 > 
 > Marek
