...
Most likely this cannot be done without slight changes to the program. You can check whether 
 ...
 The other option would be to define covariables that result in uncorrelated�  regressions in a model without a competitive effect and then assume no correlations with competitive effects.

 There is another formulation of a competition model with two effects only no matter the number of competitors. Not yet published.
