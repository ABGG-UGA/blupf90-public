## 238: Re: [blupf90] question about Gen_conflicts_all

- Title: 238: Re: [blupf90] question about Gen_conflicts_all
- Author: Andres Legarra
- Date: Jan 22, 2014
- Original: [https://groups.yahoo.com/neo/groups/blupf90/conversations/messages/238](https://groups.yahoo.com/neo/groups/blupf90/conversations/messages/238)

```
 I don't quite remember the details but this is as follows.

 If a marker gives conflict for most parent-offspring couples (say in 90% of the couples), then it is the marker that
is wrong . There is a problem with genotyping.
 If a couple sire-son has many errors (say in 25% of the marjkers), then it is either the pedigree or one of the
samples that is wrong

 Andres

Le 22/01/2014 12:49, marzieh heidaritabar a Ã©critÂ :

  Â Dear Anders,

 Thanks for the answer If this is the case, it means for each pair I have different number of conflict SNPs, so, why
the program removes 51 SNPs of the whole genome with mandelian conflicts?
 ...
 the number of
 snps that conflict between parent and progeny,
 you mean
 ...
 animal_IDÂ  sire_IDÂ Â Â Â Â  #SNP_conflictsÂ 
 %Conflicts
...
<marzieh_heidaritabar@...>
 ...
 result in the following: I do not know what do
 values 73
 and
