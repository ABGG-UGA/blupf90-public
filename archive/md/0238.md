...
```
 ...
 If a marker gives conflict for most parent-offspring couples (say in 90% of the couples), then it is the marker that is wrong . There is a problem with genotyping.
 If a couple sire-son has many errors (say in 25% of the marjkers), then it is either the pedigree or one of the samples that is wrong
...
Le 22/01/2014 12:49, marzieh heidaritabar a Ã©critÂ :
 ...
 Thanks for the answer If this is the case, it means for each pair I have different number of conflict SNPs, so, why the program removes 51 SNPs of the whole genome with mandelian conflicts?
