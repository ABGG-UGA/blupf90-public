...
The lowest eigenvalue of Dinv is 0.35. I’m still having the same pattern on variance of dominance (set to zero) and deviance in running airemlf90.
...
I bet for a numerical problem. I strongly recommend that instead of using the dominance matrix in "outfile", you invert it with an external problem (e.g. "solve' in R) and check the eigenvalues. All should be positive,  and if not do something like 0.99D+0.01 I . Then use the file with the inverted matrix and option user_file (not user_file_inv).
