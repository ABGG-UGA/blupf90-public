...
OPTION exclusion_threshold x Number of parent-progeny exclusions as precentage all SNP to determine wrong relationship. default value 2%. ==> animals were removed if failed QC.
Whether remove those possible wrong progenies in pedigree is according to OPTION verify_parentage xÂ  (if x=3 : detect and eliminate progenies with conflict; 1: only detect; 0: no action)--------------

OPTION exclusion_threshold_snp x Number of parent-progeny exclusions for each locus as percentage, of pair of genotyped animals evaluated, to exclude an SNP from the analysis. default value 10%.Â  ==> SNPs were removed if failed QC.



On Wed, Jan 22, 2014 at 6:42 AM, marzieh heidaritabar <marzieh_heidaritabar@...> wrote:
Â So, it means that the 51 wrong SNPs that was detected by program with mendelian conflicts are due to genotyping error.
