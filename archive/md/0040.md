...
```
 Hard to say, segmentation fault can be pretty much anything... it can be memory problems (here for instance), or some minor bug that goes undetected in Windows.
 You might try including and removing effects. Perhaps there is some weird character (like "ñ") that is messing up things, for instance.
