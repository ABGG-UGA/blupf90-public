...
```
 Quite honestly: rendom is a difficult program. One way is to write a program that computes the dominance relationhsip matrix (look for rules in good books of quantitative genetics like Lynch & Walsh, or Larry Schaeffer notes, or used the attached paper assuming no inbreeding), then invert it using for instance R, or Fortran, or whatever you like, and store it as a text file. Then use user_file facility in blup/remlf90 http://nce.ads.uga.edu/wiki/doku.php?id=user_defined_files_for_covariances_of_random_effects
