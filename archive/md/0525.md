...
```
I ran two-trait analyses in AIREMLF90. One trait had repeated measures whereas the another trait had only a single measure per animal. I followed the instructions of Andres Legarra of a previous post (https://groups.yahoo.com/neo/groups/blupf90/conversations/topics/178) and the analyses worked very well. The coefficients of heritability were similar to those obtained in one-trait analyses and the genetic correlation was estimated by using the OPTION se_covar_function. However, the phenotypic correlation assumed an absurd value and its standard error was not estimated. Anyone knows what can be happening? Any help will be very appreciated. Below I printed the airemlf90.log from one of these analyses. 
