...
```
 ...
 For inversion, preGSf90 will use 0.95G + 0.05 A, which in this case os 0.95G + 0.05 I . This makes matrix G invertble and the difference is negligible.
 VERY important you need to deactivate all quality control related to pedigreeÂ  and also all corrections of G to resemble A:
...
Â First, thank you for putting together a great set of programs that work very well and for allowing the community to use them.
 I am currently putting together a genomic evaluation in beef cattle. We have no observed pedigree, but we do parentage analysis from SNP data to create one. Some animals (10%) do not get assigned parentage, but are still useful through the genomic relationship matrix.Â 
